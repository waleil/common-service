package cn.net.yzl.logger.utils;
...
public class SnowFlake {
    ...
    public static String getId() {
        ...
        Long id = SnowFlake.get().nextId();
        try {
            addr = InetAddress.getLocalHost();
            String adders = addr.getHostAddress().replace(".", "");
            if (StringUtils.hasText(adders) && adders.length() > 5) {
                String start = adders.substring(adders.length() - 5, adders.length());
                adders = adders.replaceAll(adders, start);
            }
            String globleid = String.valueOf(id);
            String strtid = globleid.substring(adders.length(), globleid.length());
            return adders + strtid;
