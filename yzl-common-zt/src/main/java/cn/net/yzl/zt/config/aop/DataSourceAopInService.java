...
import cn.net.yzl.zt.config.mysql.DataSourceType;
...
//			+ " and @annotation(cn.net.yzl.bi.config.annotation.ReadDataSource) ")
	@Before("execution(* cn.net.yzl.zt.service..*.find*(..)) "
			+ " or execution(* cn.net.yzl.zt.service..*.get*(..)) "
			+ " or execution(* cn.net.yzl.zt.service..*.query*(..))"
			+ " or execution(* cn.net.yzl.zt.service..*.select*(..))")
	public void setReadDataSourceType() {
		//如果已经开启写事务了，那之后的所有读都从写库读
		if(!DataSourceType.write.getType().equals(DataSourceContextHolder.getReadOrWrite())){
			DataSourceContextHolder.setRead();
...
//			+ " and @annotation(cn.net.yzl.bi.config.annotation.WriteDataSource) ")
	@Before("execution(* cn.net.yzl.zt.service..*.insert*(..)) "
			+ " or execution(* cn.net.yzl.zt.service..*.update*(..))"
			+ " or execution(* cn.net.yzl.zt.service..*.add*(..))"
			+ " or execution(* cn.net.yzl.zt.service..*.create*(..))"
			+ " or execution(* cn.net.yzl.zt.service..*.save*(..))"
			+ " or execution(* cn.net.yzl.zt.service..*.delete*(..))")
	public void setWriteDataSourceType() {
		DataSourceContextHolder.setWrite();
	}
